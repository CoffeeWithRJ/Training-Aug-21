...
Cname VARCHAR(18) DEFAULT null CONSTRAINT FK01 FOREIGN KEY (Cname) REFERENCES CUSTOMER(Cname) ,
Bname VARCHAR(18) DEFAULT null CONSTRAINT FK02 FOREIGN KEY (Bname) REFERENCES BRANCH(Bname) ,
Amount INT not null,
Adate DATE
)
...
Cname VARCHAR(18) DEFAULT null CONSTRAINT FK03 FOREIGN KEY (Cname) REFERENCES CUSTOMER(Cname) ,
Bname VARCHAR(18) DEFAULT null CONSTRAINT FK04 FOREIGN KEY (Bname) REFERENCES BRANCH(Bname) ,
Amount INT not null
)
...
SELECT C.Cname FROM CUSTOMER C WHERE C.CITY 
IN (SELECT B.CITY FROM BRANCH B WHERE B.Bname
...
SELECT D.Cname FROM Deposit D JOIN BRANCH B ON D.Bname = B.Bname WHERE D.Bname 
IN (SELECT D.Bname FROM Deposit D JOIN BRANCH B ON D.Bname=B.Bname WHERE D.Cname = 'SUNIL' )

--Q3: List the Names of Customers Living in the City where the Maximum Number of Depositors are Located
SELECT C.Cname, C.CITY FROM CUSTOMER C WHERE C.CITY 
IN (SELECT C1.CITY FROM Deposit D JOIN CUSTOMER C1 ON C1.Cname= D.Cname
GROUP BY C1.CITY HAVING COUNT(D.Cname) >=ALL
(SELECT COUNT(D1.Cname) FROM Deposit D1 JOIN CUSTOMER C2 ON D1.Cname = C2.Cname Group BY C2.CITY) 

--Q4: List Names of Borrowers Having Deposit Amount Greater than 1000 and Loan Amount Greater than 2000
SELECT B.Cname, B.Amount, D.Cname, D.Amount FROM BORROW B JOIN Deposit D 
ON  D.Cname = B.Cname AND D.Amount > 1000 AND B.Amount > 2000
